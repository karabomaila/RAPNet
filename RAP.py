...
from modules.voxelmorph_us import SpatialTransformer, U_Network
...
class SDnetSegmentor(nn.Module):
    ...
    def __init__(self, params) -> None:
        ...
        self.conv00 = nn.Sequential(nn.Conv2d(3, 16, 5, 1, 2), nn.ReLU(inplace=True))

        self.conv11 = nn.Sequential(nn.Conv2d(3, 16, 5, 1, 2), nn.ReLU(inplace=True))

        self.conv22 = nn.Sequential(nn.Conv2d(3, 16, 7, 1, 3), nn.ReLU(inplace=True))
    ...
    def forward(self, query_image, inpt_sp, inpt_mask):
        ...
        if len(inpt_sp.shape) == 5:
            ...
            for i in range(inpt_sp.shape[0]):
                ...
                sp_mask = F.interpolate(
                    inpt_mask[i][:, 1:2, ...],
                    size=(sp_level_features.shape[-2:]),
                    mode="nearest",
                )

                corr_fg = Correlation(
                    sp_level_features * sp_mask, sp_level_features * (1 - sp_mask)
                )
                corr_bg = Correlation(
                ...
                corr_bg = self.q4(
                    torch.cat([corr_bg, sp_level_features * (1 - sp_mask)], dim=1)
                )
            ...
            sp_prior_r = F.interpolate(
                sp_prior, size=(q_level_features.shape[-2:]), mode="nearest"
            )

            corr_fg = Correlation(
                q_level_features * sp_prior_r, q_level_features * (1 - sp_prior_r)
            )
            corr_bg = Correlation(
                q_level_features * (1 - sp_prior_r), q_level_features * sp_prior_r
            )

            q_level_features_fg = self.q4(
                torch.cat([corr_fg, q_level_features * sp_prior_r], dim=1)
            )
            q_level_features_bg = self.q4(
                torch.cat([corr_bg, q_level_features * (1 - sp_prior_r)], dim=1)
            )
            ...
            q_level_features_fg = self.q3(
                torch.cat([corr_fg, q_level_features * sp_prior_r], dim=1)
            )
            q_level_features_bg = self.q3(
                torch.cat([corr_bg, q_level_features * (1 - sp_prior_r)], dim=1)
            )
            ...
            _, max_corr_e3 = torch.max(
                self.soft_max(
                    torch.cat([correlative_map_bg_e3, correlative_map_fg_e3], 1)
                ),
                dim=1,
                keepdim=True,
            )
            ...
            q_level_features_fg = self.q2(
                torch.cat([corr_fg, q_level_features * sp_prior_r], dim=1)
            )
            q_level_features_bg = self.q2(
                torch.cat([corr_bg, q_level_features * (1 - sp_prior_r)], dim=1)
            )
        ...
        attention_e4 = self.conv00(
            torch.cat([correlative_map_bg_e4, correlative_map_fg_e4, sp_prior_e4], 1)
        )

        sp_prior_e3 = F.interpolate(sp_prior, size=(e3.shape[-2:]), mode="nearest")
        attention_e3 = self.conv11(
            torch.cat([correlative_map_bg_e3, correlative_map_fg_e3, sp_prior_e3], 1)
        )

        sp_prior_e2 = F.interpolate(sp_prior, size=(e2.shape[-2:]), mode="nearest")
        attention_e2 = self.conv22(
            torch.cat([correlative_map_bg_e2, correlative_map_fg_e2, sp_prior_e2], 1)
        )
...
if __name__ == "__main__":
    ...
    def get_configs():
        parser = argparse.ArgumentParser(
            description="PyTorch Few Shot Semantic Segmentation"
        )
        parser.add_argument(
            "--config", type=str, default="./settings.cfg", help="config file"
        )
        parser.add_argument(
            "--mode",
            "-m",
        ...
        parser.add_argument("--device", "-d", default=0, help="device to run on")
